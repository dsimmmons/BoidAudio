...
public:
    ...
    void updatePosition()
    {
        ...
        const int maxBirdsInZone = p->numBirds * p->processRatio;
                ...
                if (thisYPos > -1 && thisYPos < p->yNumSpots)
                {
                    for (int xInd = -1; xInd < 2; xInd++)
                    {
                        ...
                        if (thisXPos > -1 && thisXPos < p->xNumSpots)
                        {
                            ...
                            while (b != nullptr && numBirdsInZone < maxBirdsInZone)
                            {
                                if (b->id != id)
                                {
        ...
        const float circleXDist = xPos - 0.5f;
        const float circleYDist = yPos - 0.5f;

        const float circleDist = std::max(0.001f, getDistance(0.5f, 0.5f));

        const int fftIndex = int(circleDist * p->fftWeights.size()) - 1;
        float fftWeightA = 0.0f;
        float fftWeightB = 0.0f;
        if (p->circleWeight != 0.0f && fftIndex > -1 && fftIndex < p->fftWeights.size())
        {
            fftWeightA = p->fftWeights[fftIndex];
            fftWeightB = p->fftWeights[fftIndex + 1];
            if (fftWeightA > 0.0f && fftWeightB > 0.0f)
            {
                const float circleXVec = circleXDist / circleDist;
                const float circleYVec = circleYDist / circleDist;

                xVelocity += circleYVec * p->circleSlideMod * fftWeightA * p->circleWeight * (1 - 2 * (fftWeightA > fftWeightB));
                yVelocity += circleXVec * p->circleSlideMod * fftWeightA * p->circleWeight * (1 - 2 * (fftWeightA > fftWeightB));

                xVelocity += (circleXVec * p->circlePullMod * fftWeightA * p->circleWeight) * (1 - 2 * (fftWeightA > fftWeightB));
                yVelocity += (circleYVec * p->circlePullMod * fftWeightA * p->circleWeight) * (1 - 2 * (fftWeightA > fftWeightB));
            }
        }
        ...
        if (fftWeightA > 0.0f && fftWeightB > 0.0f)
        {
            newVel += fftWeightA * p->circleSpeedMod * p->circleWeight * (1 - 2 * (fftWeightA > fftWeightB));
        }
