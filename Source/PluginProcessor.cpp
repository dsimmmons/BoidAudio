...
void NewProjectAudioProcessor::pushNextSampleIntoFifo(const float& sample)
{
        // if the fifo contains enough data, set a flag to say
        // that the next line should now be rendered..
        if (p.fifoIndex == p.fftSize)       // [8]
        {
            if (!p.nextFFTBlockReady)    // [9]
            {
                std::fill(p.fftData.begin(), p.fftData.end(), 0.0f);
                std::copy(p.fifo.begin(), p.fifo.end(), p.fftData.begin());
    ...
    p.fifo[(size_t)p.fifoIndex++] = sample; // [9]
}
